...
Create a Python evaluator for the target function named: {tool_name}
...
5. Call the target function with inputs from the training data AND compare the outputs to the training data outputs
6. If the function outputs cannot be validated deterministically, only then use an LLM to evaluate algorithmic correctness and implementation quality
...
def evaluate(program) -> dict:
    ...
    for input_data in training_data:
        try:
            ...
            # log the training data output

            # Compare the result to the training data output
            # if the result is equal to the training data output, then the score for this row is 1.0
            # if the result is not equal to the training data output, then the score for this row is 0.0
...
def get_prompt_evaluator_prompt(tool_name: str, function_description: str, 
    ...
    return f"""
    ###ROLE: 
    You are an expert at generating evaluators for LLM prompts.
    Create a Python evaluator for the target prompt named: {tool_name}

##TARGET PROMPT ANALYSIS:
{function_description}

##PROMPT/TEMPLATE
This is a prompt template. The evaluator must the prompt by:
