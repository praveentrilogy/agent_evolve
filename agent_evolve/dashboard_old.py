    ...
    for tool_dir in base_path.iterdir():
        ...
        if openevolve_output.exists():
            ...
            if tool_data['checkpoints']:
                ...
                print(f"  🔍 Looking for initial scores at: {initial_score_file}")
                print(f"  🔍 File exists: {initial_score_file.exists()}")
                if initial_score_file.exists():
                    try:
                        with open(initial_score_file, 'r') as f:
                            original_scores = json.load(f)
                            print(f"  ✅ Loaded initial scores for {tool_name}: {original_scores}")
                            ...
                            if isinstance(original_scores, dict) and 'combined_score' in original_scores:
                                ...
                                print(f"  ✅ Filtered initial scores for {tool_name}: {original_scores}")
                    except Exception as e:
                        print(f"  ⚠️ Could not load initial scores for {tool_name}: {e}")
                else:
                    print(f"  ⚠️ No initial_score.json found for {tool_name} at {initial_score_file}")
                ...
                if not original_scores and tool_data.get('evaluator_code') and tool_data.get('original_code'):
                    try:
                        ...
                        try:
                            ...
                            print(f"  ⚠️ Could not evaluate original code for {tool_name}: {e}")
                        finally:
                            # Clean up temp file
                            try:
                        ...
                        print(f"  ⚠️ Error setting up original evaluation for {tool_name}: {e}")
...
def main():
    ...
    if not tools_data:
        ...
        st.markdown("""
        To evolve your functions, add the `@evolve()` decorator to functions you want to improve:
        """)
...
def your_function(param1, param2):
    """Your function that needs improvement"""
    ...
    return result
        ...
        st.markdown("""
        You can also mark functions for evolution using comments (useful when you can't modify imports):
        """)
...
def your_function(param1, param2):
    ...
    return result
        ...
        with col1:
            st.markdown("**📊 Data Processing Function**")
            st.code('''
from agent_evolve import evolve
...
def analyze_sales_data(sales_records):
    ...
    # Simple analysis that could be improved
    total_sales = sum(record['amount'] for record in sales_records)
    return {'total': total_sales}
            ''', language='python')
        
        with col2:
            st.markdown("**🤖 AI Agent Node**")
            st.code('''
from agent_evolve import evolve

@evolve()
def research_node(state):
    """Research information based on user query"""
    query = state['messages'][-1].content
    # Basic research logic that could be enhanced
    ...
    return {'research': results}
        ...
        st.markdown("""
        After adding decorators to your code, click the button below to extract and set up your evolution targets:
        """)
        
        # Add prominent extraction button
            ...
            with st.spinner("🔄 Scanning your code for evolution targets..."):
                try:
                    ...
                    if search_path.exists():
                        ...
                        st.info(f"📁 Scanning {len(python_files)} Python files in {search_path}")
                        ...
                        for py_file in python_files:
                            try:
                                ...
                                if decorated_tools or commented_tools:
                                    st.success(f"✅ Found {len(decorated_tools) + len(commented_tools)} targets in {py_file.name}")
                                
                            except Exception as e:
                                st.warning(f"⚠️ Error processing {py_file.name}: {e}")
                        
                        if total_extracted > 0:
                            st.success(f"🎉 Successfully extracted {total_extracted} evolution targets!")
                            st.info("🔄 Refreshing dashboard...")
                            st.rerun()
                        else:
                            st.warning("⚠️ No evolution targets found. Make sure you've added @evolve() decorators or #@evolve() comments to your functions.")
                            st.info("💡 Tip: Check the examples above and make sure your decorators are exactly `@evolve()` with parentheses.")
                    else:
                        st.error(f"❌ Source directory not found. Searched: {search_path}")
                        st.info("💡 Make sure you have a `src/` directory with your Python code, or run the dashboard from your project root.")
                except Exception as e:
                    st.error(f"❌ Extraction failed: {e}")
                    st.info("💡 Try checking the troubleshooting section below for common issues.")
        
        # Step 4: Next steps
        st.markdown("### 5️⃣ Next Steps After Sync")
        st.markdown("""
        Once your targets are extracted, you'll be able to:
        - **📋 Generate training data** for your functions
        - **🔧 Create evaluators** to measure performance
        - **🧬 Run evolution** to improve your code
        - **📊 Track progress** and compare versions
        """)
        ...
        with st.expander("🔧 Troubleshooting", expanded=False):
            ...
            **Common Issues:**
            ...
            1. **"No evolution targets found"** after sync:
               - Make sure you've added `@evolve()` decorators or `#@evolve()` comments
               - Check that your code is in a `src/` directory
               - Ensure your Python files are syntactically correct
            
            2. **Import errors**:
               - Install agent-evolve: `pip install agent-evolve`
               - Add the import: `from agent_evolve import evolve`
            
            3. **Functions not detected**:
               - Decorators must be exactly `@evolve()` (with parentheses)
               - Comments must be exactly `#@evolve()` on the line before the function
               - Functions must be properly indented Python code
            
            4. **Directory not found**:
               - The sync looks for `src/` directory relative to your project
               - Make sure you're running from your project root directory
            """)
            ...
            with st.spinner("🔄 Syncing tools..."):
                try:
                    ...
                    if search_path.exists():
                        ...
                        st.success(f"✅ Synced {total_extracted} evolution targets!")
                        st.rerun()
                    else:
                        st.error(f"❌ Source directory not found. Searched: {search_path}")
                except Exception as e:
            ...
            with st.spinner("🔄 Syncing tools..."):
                try:
                    ...
                    if search_path.exists():
                        ...
                        if total_extracted > 0:
                            st.success(f"✅ Found {total_extracted} evolution targets!")
                            st.rerun()
                        else:
                            st.warning("No evolution targets found. See instructions below.")
                    else:
                        st.error(f"❌ Source directory not found. Searched: {search_path}")
                except Exception as e:
                    st.error(f"❌ Sync failed: {e}")
...
def research_node(state):
    ...
    return {'research': results}
            ...
            st.markdown("### 4️⃣ Extract Your Evolution Targets")
            st.markdown("""
            After adding decorators to your code, click the **🚀 Get Started - Sync Tools** button in the sidebar 
            to extract and set up your evolution targets.
            """)
    ...
    with overview_tab:
        ...
        if tool_data.get('has_evolution'):
            ...
            if config_file.exists():
                st.warning("❌ Evolution not run yet")
                st.info("💡 Go to the Evolution tab to run evolution")
            else:
    ...
    with evolution_tab:
        ...
        if openevolve_output.exists():
            st.subheader("📊 Current Evolution Status")
    ...
    with evaluator_tab:
        ...
        if tool_data['evaluator_code']:
            ...
            if tool_data.get('training_data'):
                st.info("✅ Training data is available. You can now generate an evaluator.")
                
                if st.button("🚀 Generate Evaluator", type="primary", key=f"generate_evaluator_{selected_tool}"):
                    with st.spinner("🤖 Generating evaluator..."):
                        try:
                            ...
                            if not os.getenv('OPENAI_API_KEY'):
                                st.error("❌ Error: OPENAI_API_KEY environment variable is required")
                                st.info("💡 Please set your OpenAI API key: export OPENAI_API_KEY=your_key_here")
                            else:
                                ...
                                if evaluator_file.exists():
                                    ...
                                    with open(evaluator_file, 'r', encoding='utf-8') as f:
                                        tool_data['evaluator_code'] = f.read()
                                    st.success("✅ Evaluator generated successfully!")
                            ...
                            st.error(f"❌ Error generating evaluator: {e}")
                            import traceback
                            st.code(traceback.format_exc())
            else:
